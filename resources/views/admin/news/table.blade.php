            ...
            @foreach ($news as $news)
                <tr class="delete_row">
                    <td class="text-center">
                        <label class="container-checkbox">
                        <input type="checkbox" class="checkSingle" id="{{ $news->id }}">
                        <span class="checkmark"></span>
                        </label>
                    </td>
                    <td>{{ $news->content }}</td>
                    ...
                    <td class="product-action"> 
                        ...
                        <span class="delete-row btn btn-danger btn-sm" data-url="{{ url('admin/news/' . $news->id) }}"><i class="feather icon-trash"></i>{{ __('admin.delete') }}</span>
                    </td>
                </tr>
            @endforeach
        </tbody>
...
@if ($news->count() > 0 && $news instanceof \Illuminate\Pagination\AbstractPaginator )
